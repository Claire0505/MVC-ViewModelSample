...
using MVC_ViewModelSample.ViewModels;

//參考文章:http://kevintsengtw.blogspot.com/2012/12/aspnet-mvc-viewmodel.html
...
namespace MVC_ViewModelSample.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            if (id.HasValue)
            {
                var category = db.Categories.FirstOrDefault(x => x.CategoryID == id.Value);
                var products = db.Products.Where(x => x.CategoryID == id.Value)
                    .OrderBy(x => x.ProductID)
                    .ToList();


                CategoryDetailsViewModel viewModel = new CategoryDetailsViewModel();
                viewModel.CategoryData = category;
                viewModel.ProductCollection = products;
